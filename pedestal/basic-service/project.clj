(defproject basic-service "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "basic-service.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'basic-service.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init basic-service.service/service #'basic-service.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} basic-service.server)
