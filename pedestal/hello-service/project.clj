(defproject hello-service "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "hello-service.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'hello-service.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init hello-service.service/service #'hello-service.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} hello-service.server)
